...
| TL-1 | **The Letter: real subscriptions + double opt-in** — `letter_subscribers` (migration `0014`), subscribe/confirm routes, letter-scope unsubscribe tokens in the shared endpoint, confirmation + welcome templates (`letter@` sender, CAN-SPAM postal footer), homepage form wired for real | ✅ complete — merged 9c49a16, 0014 applied, supervised end-to-end test delivered + verified 2026-07-12; public form live |
| TL-2 | **The Sunday issue builder** — `letter_issues`/`letter_sends` (migration `0015`), week-in-numbers generation from our own store (snapshots + `days_on_market<=7` counts), Letter Desk editor + risky-claims lint, test-send-to-admin-only, typed-confirmation send (`SEND <date>`) with per-recipient unsubscribe tokens/headers + UNIQUE(issue, subscriber) send log; NO auto-send/cron/Claude/archive/segmentation/batch API | ⏳ implementation in PR — merge, 0015 application, supervised test-send, and the FIRST real send are each separate gates |
| Next | ⚠ Compliance copy sign-off (broker + NTREIS/Cotality), TL-3 (public issue archive), CRM webhook, compare view, instant-tier search alerts, optional Google-basemap swap (needs user's Maps API key + billing) | ⬜ |
